...
Для работы проекта необходимо создать файл `.env` в папке `backend` или задать переменные через систему. Ниже приведен список всех используемых переменных:

| Переменная | Описание | Значение по умолчанию |
|------------|----------|----------------------|
| `SECRET_KEY` | Секретный ключ Flask для сессий | автоматическая генерация |
| `JWT_SECRET` | Секретный ключ для JWT токенов | автоматическая генерация |
| `SUPER_ADMIN_PASSWORD` | Мастер-пароль для админ-панели | `SkySwag675810` |
| `CACHE_ENABLED` | Включение кэширования API | `False` |
| `CACHE_TTL` | Время жизни кэша (секунды) | `300` |
| `DEFAULT_PAGE_SIZE` | Товаров на странице по умолчанию | `20` |
| `MAX_PAGE_SIZE` | Максимум товаров на странице | `100` |
| `LOG_LEVEL` | Уровень логирования | `INFO` |

> ⚠️ Для продакшн-среды обязательно задайте свои значения `SECRET_KEY` и `JWT_SECRET` через переменные окружения.
